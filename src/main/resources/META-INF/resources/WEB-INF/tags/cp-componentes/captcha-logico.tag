...
<%@ attribute name="nameRepostas"	type="java.lang.String" 	required="true" 	rtexprvalue="true" 	description="Nome do atributo 'String' a ser inserido no 'name' de todos os radios. Normalmente ele deve referenciar algum atributo do 'backend bean' do formulario." %>
<%@ attribute name="classDivContent"					required="false" 	rtexprvalue="true" 	description="Nome da classe CSS para estilizacao do div content. Este div esta em volta de todos os componentes." %>
...
<div id="divCaptchaLogico" class="${classDivContent}">
	...
	<span class="${classSpanRespostas}">
		<c:forEach var="resposta" items="${perguntaRespostas.respostas}" varStatus="count">
			<input type="radio" id="resposta_${count.index}" name="${nameRepostas}" value="${resposta.texto}" /> ${resposta.texto}
			<br/>
		</c:forEach>
	</span>
